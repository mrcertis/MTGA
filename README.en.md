...
1. **Output styles** (`MTGA` for Russian, `MTGA-EN` for English) — Claude answers, reports, and explains in Trump-tweet style. Each language includes 88 contextual phrases for openings, status updates, results, criticism, blockers, and endings, plus a rotation rule that prevents replies from collapsing into one repeated catchphrase. File paths, line numbers, test results, and honesty stay fully intact. The style applies ONLY to chat text: code, files, docs, and commands are never touched.
